...
export default function MyArticleDetail({ params }) {
  ...
  if (articleDetail) {
    const relatedArticles = findRelatedArticles(articles, articleDetail, 3);

    return (
      <>
        <div className="flex flex-col">
          <h1 className="gcHeading2p text-gcPrimary-1000 mt-5 sm:px-0 px-4">
            {article.title}
          </h1>
          <div className="mt-5 flex sm:flex-row flex-col lg:gap-10 sm:gap-8 gap-6">
            <div className="sm:w-3/4 w-full flex flex-col">
              <img
                className="sm:rounded-xl object-cover xl:h-[400px] lg:h-[350px] md:h-[300px] h-[200px]"
                src={`${hostNoPrefix}uploads/${article.image}`}
                alt={article.slug}
              />
              <div className="flex flex-row items-start justify-between mt-5 sm:px-0 px-4">
                <div className="flex flex-row justify-center items-center gap-3">
                  <img
                    className="rounded-full lg:w-12 md:w-11 sm:w-10 w-9"
                    src="https://placehold.co/50x50"
                    alt="Profile Author"
                  />
                  <div>
                    <h4 className="gcContentAccent1p text-gcPrimary-1000">
                      Jane Doe
                    </h4>
                    <h6 className="gcContentBody2p text-gcSecondary-400 xl:-mt-1">
                      Botanist
                    </h6>
                  </div>
                </div>
                <h4 className="gcContentAccent2p text-gcPrimary-1000">
                  6 July 2024
                </h4>
              </div>
            ...
            <div className="sm:w-1/4 w-full flex flex-col sm:px-0 px-4">
              ...
              {relatedArticles.map((article) => {
                return (
                  <div
                    key={article.id}
                    className="flex xl:flex-row flex-col-reverse justify-between sm:justify-center xl:items-center items-start xl:py-4 py-2 border-b border-gcSecondary-400 gap-3"
                  >
